...
GlutApp::GlutApp(const char* label, int x, int y, int w, int h) {
    ...
    std::cout << "Welcome to Pongnite" << std::endl;
    std::cout << "Player 1: left and right keys" << std::endl << std::endl;
    std::cout << "Player 2: A and D  keys" << std::endl << std::endl;
    std::cout << "P = pause   S = save   L = load   Space bar = reset   ESC = exit";
}
